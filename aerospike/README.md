...
```console
docker run -d -v DIR:/opt/aerospike/etc/ -e "FEATURE_KEY_FILE=/opt/aerospike/etc/features.conf" --name aerospike -p 3000:3002 aerospike:ee-[version]
```
...
### Running an Aerospike EE node with a feature key file in an environment variable

```console
FEATKEY=$(base64 ~/Desktop/evaluation-features.conf)
docker run -d -e "FEATURES=$FEATKEY" -e "FEATURE_KEY_FILE=env-b64:FEATURES" --name aerospike -p 3000:3002 aerospike:ee-[version]
```

### Running an Aerospike CE node

```console
docker run -d --name aerospike -p 3000:3002 aerospike:ce-[version]
```
...
```console
docker run -d --name aerospike -e "NAMESPACE=demo" -p 3000:3002 -v /my/dir:/opt/aerospike/etc/ -e "FEATURE_KEY_FILE=/opt/aerospike/etc/features.conf" aerospike:ee-[version]
```
...
```console
docker run -d -v /opt/aerospike/etc/:/opt/aerospike/etc/ --name aerospike -p 3000:3002 aerospike:ee-[version] --config-file /opt/aerospike/etc/aerospike.conf
```
...
```console
docker run -d  -v /opt/aerospike/data:/opt/aerospike/data  -v /opt/aerospike/etc:/opt/aerospike/etc/ --name aerospike -p 3000:3002 -e "FEATURE_KEY_FILE=/opt/aerospike/etc/features.conf" aerospike:ee-[version]
```
...
```console
docker run -d -v /opt/aerospike/data:/opt/aerospike/data -v /opt/aerospike/etc/:/opt/aerospike/etc/ --name aerospike -p 3000:3002 aerospike:ee-[version] --config-file /opt/aerospike/etc/aerospike.conf
```
...
```console
docker run -d --device '/dev/sdc:/dev/xvdc' -v /opt/aerospike/etc/:/opt/aerospike/etc/ --name aerospike -p 3000:3002 aerospike:ee-[version] --config-file /opt/aerospike/etc/aerospike.conf
```
...
```sh
docker run -d -v /opt/aerospike/lua:/opt/aerospike/usr/udf/lua -v /opt/aerospike/data:/opt/aerospike/data --name aerospike -p 3000:3002 -e "FEATURE_KEY_FILE=/opt/etc/aerospike/features.conf" aerospike:ee-[version]
```
...
These tags are for Aerospike EE images, and will require a feature key file,
such as the one you get with the single-node EE evaluation, or one associated
with a commercial enterprise license agreement.

### ce-[version]

These tags are for Aerospike CE images, and do not require a feature key file to
start. As mentioned above, the developer API for both is the same, but the
